...
const videoName=job.videoName??(width*9===height*16?'master-16x9.mp4':width*16===height*9?'master-9x16.mp4':`master-${width}x${height}.mp4`);
const audioProfile=resolve(root,'audio.json'), songProfile=resolve(root,'song-profile.json'), direction=resolve(root,'direction.json'), stylePlan=resolve(root,'style-plan.json'), timelineValidation=resolve(root,'timeline-validation.json'), video=resolve(job.videoOutput??resolve(root,videoName)), videoValidation=resolve(root,'delivery-validation.json');
...
const renderArgs=['--timeline',direction,'--style-plan',stylePlan,'--audio-data',audioProfile,'--audio',audio,'--start',String(start),'--from','0','--duration',String(duration),'--timeline-duration',String(duration),'--width',String(job.width??1920),'--height',String(job.height??1080),'--fps',String(fps),'--out',video]; if(job.workers) renderArgs.push('--workers',String(job.workers)); run('render-browser-sample.mjs',renderArgs);
const stylePlanData=JSON.parse(readFileSync(stylePlan,'utf8'));
const validationArgs=['--input',video,'--timeline',direction,'--fps',String(fps),'--width',String(width),'--height',String(height),'--out',videoValidation]; if(stylePlanData.backgroundMode==='black') validationArgs.push('--allow-black-background'); run('validate-rendered-mv.mjs',validationArgs);
writeFileSync(resolve(root,'delivery-manifest.json'),JSON.stringify({schemaVersion:1,job:resolve(jobPath),audio,timeline,artifacts:{audioProfile,songProfile,direction,stylePlan,timelineValidation,video,videoValidation},status:'passed'},null,2));
